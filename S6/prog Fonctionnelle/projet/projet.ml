...
(*val setLink : ('a * 'b * 'c * 'd) list -> 'a -> 'c -> ('a * 'b * 'c * 'd) list =  <fun> *)
...
(*val setOnstack : ('a * 'b * 'c * 'd) list -> 'a -> 'd -> ('a * 'b * 'c * 'd) list = <fun> *)
...
(*val getIndex : ('a * 'b * 'c * 'd) list -> 'a -> 'b = <fun>*)
...
(*val getLink : ('a * 'b * 'c * 'd) list -> 'a -> 'c = <fun> *)
...
(*val getOnstack : ('a * 'b * 'c * 'd) list -> 'a -> 'd = <fun> *)
  ...
  let rec forSucc succes low = match succes with
	  s::r -> if (getIndex low s) = -1 then let low = strongconnect graphe low s pile in
		  let low = setLink low s (min (getLink low s) (getLink low sommet)) in forSucc r low
			  (*si le successeur est sur la pile, alors il fait partie de la composante connexe du sommet actuel*)
...
let tarjan graphe =
  ...
  let pile = [] in
	  ...
	  (*cette fonction sert juste a traiter chaque sommet qui n'a pas ete traite*)
